...
fn hosted_http_mcp_url(package: &ExtensionPackage) -> Option<&str> {
    // InstalledLocal (operator-installed volume) and UserRegistered providers
    // are discovery-eligible alongside compiled-in HostBundled ones (P2b).
    // Discovery still runs under the caller's scope and leases the caller's
    // credential.
    if !matches!(
