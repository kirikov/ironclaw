...
fn readiness_diagnostic_unknown_wire_variants_deserialize_safely() {
    ...
    assert_eq!(
        ...
        RebornReadinessDiagnosticComponent::Unknown
    );
    assert_eq!(diagnostic.reason, RebornReadinessDiagnosticReason::Unknown);
    assert_eq!(diagnostic.status, RebornReadinessDiagnosticStatus::Unknown);
...
fn production_wiring_issue_kinds_map_to_stable_readiness_reasons() {
    assert_eq!(
        RebornReadinessDiagnosticReason::from_production_wiring_issue_kind(
...
fn production_wiring_components_keep_host_runtime_stable_names() {
    for component in [
        ...
        let readiness_component =
            RebornReadinessDiagnosticComponent::from_production_wiring_component(component);
        let serialized = serde_json::to_value(readiness_component).unwrap();

        assert_eq!(serialized, json!(component.as_str()));
    }
}
